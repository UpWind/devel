...
//        qDebug() << "new instance of short navigation";
//        return instance;
//    } else{
...
//void ShortNavigation::clear()
//{
//    //do some clearing
//}

//void ShortNavigation::loadChartObjects(QVector<ChartObjectInterface*> cObjects) {
//    qDebug()<<"void ShortNavigation::loadChartObjects(QVector<ChartObjectInterface*> cObjects)";
//    this->chartObjects = cObjects;

////    for(int i=0; i < this->chartObjects.size(); i++)
////    qDebug()<<"ChartObjects:"<<this->chartObjects.at(i)->getTableName();// << this->chartObjects.at(i)->getPixelGeometry();
...
//    qDebug()<<"Chartobjects size: " << this->chartObjects.size();

//    for (int i = 0 ; i < this->chartObjects.size(); i++) {
//        qDebug()<<"Table name:" << this->chartObjects.at(i)->getTableName();
...
//            }
//        }
//        qDebug()<<"Point_obstacles: " << this->pointObstacles.size();
...
//     qDebug() << "QPointF ShortNavigation::getFromWKTPoint( QString wkt_geometry)";
...
//    qDebug()<< "QString ShortNavigation::buildWKTPolygon( const QPolygonF &epolygon, const QPointF &ecentroid, const double &perimeter, const float &offset )";
...
//    qDebug() << "NO1: bool ShortNavigation::checkIntersection( const QString &layerName, const QString &object, QString shape = QString()";
...
//    if ( shape.isEmpty()  ) {
//        qDebug()<<"step4: if haara" << layerName;
//        sql.append( layerName);
//        qDebug()<<"Appended with layername;"<<sql;
//        sql.append( " ) AS intersection ");
//        qDebug()<<"sql " << sql;
//    } else {
//        qDebug()<<"step5: else haara";
//        sql.append( layerName);
...
//    qDebug()<<"FUll SQL: " << sql;
//    QByteArray arr = sql.toAscii();
//    sql = "SELECT * FROM ";
//    qDebug()<< "array: " << arr;
//    qDebug()<<"Connection: " <<this->conn;
//    try{
//         res = PQexec(conn, arr/*sql.toAscii()*/ );
//    }catch(std::exception& e){
//         qCritical() << "Exception thrown:" << e.what();
//    }
//    qDebug()<<"SQL to ascii succesfull";
//    bool intersection = ( PQntuples(res) > 0 );
...
//    qDebug() << "NO2: bool ShortNavigation::checkIntersection( const QString &layerName, const QPolygonF &triangle, const QPolygonF &rhomboid )";
...
//    qDebug()<< "NO3: bool ShortNavigation::checkIntersection( const QString &layerName, const QPointF &point )";
...
//    qDebug()<<"ShortNavigation::getPath";
...
//        qDebug()<<"Get path1: calling checkIntersection with 3 param";
...
//                qDebug()<<"Get path2: calling checkIntersection with 3 param";
...
//    qDebug()<<"QPointF ShortNavigation::getNextPoint( const QVector<QPointF> &route, const QPointF &boatPos, const float &offset)";
...
//            qDebug()<<"getNextPoint: calling checkIntersection with 3 param";
...
//    qDebug()<<"void ShortNavigation::updateCheckPoint(";
...
//        qDebug()<<"Get path: calling checkIntersection with 3 param";
...
//        if (debug) qDebug() << "processData(): WARNING: tables are not done, trying to do them...";
...
//            if (debug) qDebug() << "processData(): FATAL: failed to create the tables";
...
////        if (debug) qDebug() << "processData(): starting data processing...";
...
////        if (debug) qDebug() << "processData(): data processed and signal send";
